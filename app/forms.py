...
class StudForm(forms.Form):
    ...
    second_name = forms.CharField(max_length=35,label="Surname",
        widget=forms.TextInput(
            attrs={"placeholder":"Enter student surname"}
        ))
    patronymic = forms.CharField(max_length=25,
        widget=forms.TextInput(
            attrs={"placeholder":"Enter student patronymic"}
        ))
    date_of_birth = forms.DateField(
        input_formats=["%d.%m.%Y","%d/%m/%Y","%d-%m-%Y"],
        widget=forms.TextInput(
            attrs={"placeholder":"Enter student birthday"}
        ))
...
class GroupForm(forms.Form):
    ...
    warden = forms.ModelChoiceField(queryset=Student.objects.all(),
                                    empty_label="Choose warden",
                                    required=False,
                                    widget=forms.Select)
